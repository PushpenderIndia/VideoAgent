...
def main():
    ...
    # Run ADK terminal from the project root with virtual environment activated
    if os.name == 'nt':  # Windows
        activate_script = "venv\\Scripts\\activate.bat"
        cmd = f"{activate_script} && adk run adk_agents"
        subprocess.run(cmd, shell=True, cwd=".")
    else:  # Unix/MacOS
        activate_script = "venv/bin/activate"
        cmd = f"source {activate_script} && adk run adk_agents"
        subprocess.run(cmd, shell=True, cwd=".")
